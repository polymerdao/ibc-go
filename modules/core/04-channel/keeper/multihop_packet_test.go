...
func (suite *MultihopTestSuite) TestRecvPacket() {
	...
	testCases := []channelTestCase{
		...
		{"channel not found", false, func() {
			...
			suite.SetupChannels()
			packet, packetHeight, err = suite.A().
				SendPacketWithInvalidDestPortDestChannel(defaultTimeoutHeight, disabledTimeoutTimestamp, ibctesting.MockPacketData)
			channelCap = suite.Z().Chain.GetChannelCapability(
				suite.Z().ChannelConfig.PortID, suite.Z().ChannelID,
			)
		}, false},
		...
		{"packet source port ≠ channel counterparty port", false, func() {
			...
			packet, packetHeight, err = suite.A().
				SendPacketWithInvalidSourcePort(defaultTimeoutHeight, disabledTimeoutTimestamp, ibctesting.MockPacketData)
		...
		{"packet source channel ID ≠ channel counterparty channel ID", false, func() {
			...
			packet, packetHeight, err = suite.A().
				SendPacketWithInvalidSourceChannel(defaultTimeoutHeight, disabledTimeoutTimestamp, ibctesting.MockPacketData)
