...
func (ep *EndpointM) ChanOpenInit() error {
	...
	require.NoError(ep.Chain.TB, err, "could not retrieve channel id from event")
...
func (ep *EndpointM) ChanOpenTry(chanInitHeight exported.Height) error {
	...
	if ep.ChannelID == "" {
		ep.ChannelID, err = ParseChannelIDFromEvents(res.GetEvents())
		require.NoError(ep.Chain.TB, err, "could not retrieve channel id from event on chain %s", ep.Chain.ChainID)
	}
